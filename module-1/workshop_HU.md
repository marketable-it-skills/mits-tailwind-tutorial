# 1. modul workshop: Projektalap + első Tailwind osztályok + navigáció + hero

**Cél**: A workshop végére lesz egy futó Vite-projekted Tailwind CSS v4-gyel, egy CSS-ben definiált egyedi dizájnrendszerrel, egy stílusozott navigációs sávval és egy teljes képernyős hero szekcióval.
...
- `@tailwindcss/vite` — a Vite-bővítmény, amely integrálja a Tailwindet a build-folyamatba (kiváltja a régi PostCSS-beállítást)

### 1.3. lépés: A Vite-bővítmény konfigurálása
...
Nyisd meg a `http://localhost:5173` oldalt. Egy egyszerű, formázatlan "VadonSzava" feliratot kell látnod. A Tailwind csatlakoztatva van — csak még semmi látható dolgot nem csinál, mert a `<h1>` elemnek nincsenek segédosztályai.
...
## 2. feladat: Első segédosztályok + Build-folyamat

### 2.1. lépés: A cím formázása segédosztályokkal
...
Mentés. A böngésző azonnal frissül (HMR — Hot Module Replacement). Egy nagy, félkövér, zöld feliratot kell látnod kitöltéssel.
...
Mentés. Egy formázott alcím jelenik meg a cím alatt.

> **Első siker!** Két HTML-elemet formáztál anélkül, hogy egyetlen sor CSS-t írtál volna. Ez a Tailwind módszere.
...
> **Kulcspont:** Az `npm run dev` a munkakörnyezeted (gyors, HMR-rel). Az `npm run build` hozza létre a telepíthető kimenetet. Vizuálisan ugyanazt az eredményt adják — csak a kézbesítési mechanizmus különbözik.

---

## 3. feladat: Dizájntokenek az `@theme {}` segítségével

Jelenleg a Tailwind beépített színeit használod, mint a `text-green-700`. De a túrablogunknak szüksége van egy konkrét márkazöldre (`#2e7d32`) és egyedi barna árnyalatokra, amelyek nem léteznek a Tailwind alapértelmezett palettájában.

Először nézzük meg a problémát.

### 3.1. lépés: Próbálj ki egy egyedi színt token nélkül

Változtasd meg a `text-green-700` osztályt egy beégetett értékű style attribútumra:
...
- Az érték mindenhol megismétlődik, ahol szükséges ez a szín
...
### 3.2. lépés: Egyedi paletta-tokenek hozzáadása

Nyisd meg a `src/style.css` fájlt és bővítsd ki az `@theme {}` blokkal:
...
A cím most a márkánk zöldjével van formázva. A lényeg: az `@theme {}` blokkba kerülő minden elem automatikusan Tailwind segédosztállyá válik. A `--color-tura-green-700` `text-tura-green-700`-dá, `bg-tura-green-700`-dá, `border-tura-green-700`-dá válik stb.

### 3.3. lépés: Betűtípus-tokenek és Google Fonts hozzáadása

A blog az Inter (sans-serif) és a Merriweather (serif) betűtípusokat használja a Google Fontsból. Add hozzá a betűtípus-importot az `index.html` fájlban a `<head>` belsejébe, és adj betűtípus-tokeneket az `@theme {}` blokkhoz a `style.css` fájlban.
...
Ezek standard CSS-változók a `:root` elemre. Értékként a paletta-tokeneket használják.

### 3.5. lépés: Szemantikus tokenek leképezése Tailwind segédosztályokra

A CSS-változók nem válnak automatikusan Tailwind segédosztályokká. Szükség van még egy lépésre — az `@theme inline {}` blokkra:

```css
/* Szemantikus tokenek leképezése Tailwind segédosztályokra */
@theme inline {
...
Az `@theme inline` azt mondja a Tailwindnek: „generálj segédosztályokat ezekhez a tokenekhez, de old fel őket futásidőben, hogy a CSS-változó értéke megváltoztatható legyen (pl. amikor `.dark` kerül a `<html>` elemre)."
...
A `scroll-smooth` engedélyezi a sima görgetést hivatkozásokra kattintáskor. A `bg-bg` és a `text-text` a szemantikus tokeneket használja az oldal háttérszínéhez és az alapértelmezett szövegszínhez.
...
- `fixed w-full z-50` — az oldal tetejéhez rögzítve, teljes szélesség, minden tartalom felett
...
### 4.4. lépés: Navigációs hivatkozások hozzáadása (asztali)

A logó `<div>` után add hozzá az asztali navigációs hivatkozásokat:
...
A `hidden md:flex` azt jelenti: alapértelmezetten rejtve (mobil), flex sorként megjelenítve közepes és szélesebb képernyőkön. Méretezd át a böngészőt, hogy lásd, hogyan jelennek meg és tűnnek el a hivatkozások.

### 4.5. lépés: A mobil hamburger gomb hozzáadása

Az asztali hivatkozások `<div>` után add hozzá:
...
- **Széles (≥768px)**: Logó + szöveges hivatkozások + a "Rólam" pill gomb látható
...
- `relative` — pozicionálási kontextust teremt a fedőréteg számára (következő lépés)

### 5.3. lépés: A sötét fedőréteg hozzáadása
...
A fedőréteg `<div>` után add hozzá:
...
A `relative z-10` ezt a divet a fedőréteg fölé helyezi (a fedőréteg az alapértelmezett z-indexen van). A `text-5xl md:text-7xl` nagy főcimet eredményez asztali gépen, kissé kisebbet mobilon.
...
- **Másodlagos**: félig átlátszó fehér (üveges hatás, glassmorphism) `backdrop-blur-sm`-mel
...
- A sötét fedőréteg látható (a fotó sötétebb, mint a forráskép)
...
Ez megakadályozza a `node_modules/` mappa (több száz MB) és a lefordított kimenet commitolását.
...
### 6.4. lépés: A munka indexelése és commitolása

```bash
git add .
git commit -m "1. modul befejezve: projektbeállítás, dizájntokenek, navigáció és hero"
```

### 6.5. lépés: Váltás a main ágra és összefésülés
...
Látogasd meg a GitHub-on lévő repository-dat. Ott kell látnod az összes projektfájlt.

> **Tipp:** Commitolj minden nagyobb feladat elvégzése után, ne csak a modul végén. Az egyértelmű commit üzenetek segítenek megérteni a projekt előzményeit.
...
- ✅ Első segédosztályok megírva és megértve
- ✅ Éles build (`dist/`) létrehozva és előnézve
- ✅ Egyedi dizájntoken-rendszer definiálva az `@theme {}` segítségével
- ✅ Reszponzív, rögzített navigációs sáv
- ✅ Teljes képernyős hero háttérképpel, fedőréteggel és CTA gombokkal
...
Valósítsd meg a mobil hamburger menü kapcsolóját. A gomb már renderelve van — adj hozzá egy kattintáskezelőt a `src/main.js` fájlban, amely megmutatja és elrejti a mobil navigációs menüt, amikor a gombra kattintanak.

Tippek:
- Add hozzá a `<div id="mobileMenu" class="hidden md:hidden ...">` divet a nav sáv alá ugyanazokkal a hivatkozásokkal
